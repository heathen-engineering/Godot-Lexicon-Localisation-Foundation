...
class_name LexiconAssetCompactEditor

## One-row compact editor for LexiconAsset: Mode | (key picker when Localised, or an
## EditorResourcePicker for literal_asset when Literal/Invariant).
...
func _update_property() -> void:
	...
	if is_localised:
		...
		_resource_picker.edited_resource = obj.get("literal_asset")
...
func _on_resource_changed(resource: Resource) -> void:
	if _updating:
		return
	emit_changed("literal_asset", resource)
